...
Primero corremos el modelo usando la totalidad de los datos

```{r}
cfa_ActivoPasivo <- "activo =~ A1 + A2 + A3 + A4
pasivo =~ P1 + P2 + P3"
cfa_Generico <- 'uso =~ A1 + A2 + A3 + A4 + P1 + P2 + P3'
cfa_model <- cfa(cfa_ActivoPasivo, data = spam, mimic =c("MPlus"), std.lv = TRUE, ordered = TRUE)
fitMeasures(cfa_model, c("chisq.scaled", "df", "pvalue.scaled"))
fitMeasures(cfa_model, c("rmsea.scaled", "rmsea.ci.lower.scaled", "rmsea.ci.upper.scaled", "rmsea.pvalue.scaled"))
fitMeasures(cfa_model, c("cfi.scaled", "srmr"))
```
